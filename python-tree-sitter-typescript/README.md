...
The tree-sitter TypeScript grammar as an importable `tree_sitter_typescript` module.
...
`PYTHONPATH`, so it loads two grammars (TypeScript and TSX) from the wheel under test rather than from a
